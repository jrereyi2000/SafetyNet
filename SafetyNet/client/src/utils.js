...
export const getRandomName = () => {
  return `${getRandomString(randomIntFromInterval(4, 8))} ${getRandomString(randomIntFromInterval(4, 8))}`;
};
...
export const formatTime = (time) => {
  if (Object.prototype.toString.call(time) !== '[object Date]') time = new Date(time);
...
export const getAddress = async (longitude, latitude) => {
  const res = await axios.get(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${latitude},${longitude}&key=${Config.GOOGLE_API_KEY}`);
